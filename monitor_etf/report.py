...
def _tax_block(tax: Optional[Dict[str, float]]) -> str:
    ...
    return (
        ...
        f'<h4 style="margin:0 0 10px;color:#1B2A4A;font-size:13px;text-transform:uppercase">💶 Si vendieras hoy — Impacto fiscal</h4>'
        f'<table style="width:100%;font-size:13px;border-collapse:collapse">'
        f'<tr><td style="color:#666;padding:3px 0">Ganancia/pérdida bruta</td>'
        f'<td style="text-align:right;font-weight:600;color:{color}">{sign}{tax["gain"]:,.0f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Impuesto a pagar (IRPF)</td>'
        f'<td style="text-align:right;font-weight:600;color:#C0392B">−{tax["tax"]:,.0f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0"><strong>Dinero real a bolsillo</strong></td>'
        ...
        f'⚠️ <strong>Guarda {tax["set_aside"]:,.0f} € aparte</strong> si vendes.</p></div>'
    )
...
def _projection_block(proj: Optional[Dict[str, Any]], accent: str) -> str:
    ...
    return (
        ...
        f'<h4 style="margin:0 0 10px;color:#1B2A4A;font-size:13px;text-transform:uppercase">📊 Seguimiento del plan — Año {year}</h4>'
        f'<table style="width:100%;font-size:13px;border-collapse:collapse">'
        f'<tr><td style="color:#666;padding:3px 0">Valor proyectado (año {year})</td>'
        f'<td style="text-align:right;font-weight:600">{proj["expected"]:,.0f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Valor real actual</td>'
        f'<td style="text-align:right;font-weight:600">{proj["actual"]:,.0f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0"><strong>Desviación vs plan</strong></td>'
...
def _status_card(
    ...
    return (
        ...
        f'<div style="font-size:10px;color:#888;text-transform:uppercase">Valor cartera</div>'
        f'<div style="font-size:15px;font-weight:700;color:#1B2A4A">{portfolio_val:,.0f} €</div></div>'
        f'<div style="background:#F5F8FF;border-radius:6px;padding:10px">'
        f'<div style="font-size:10px;color:#888;text-transform:uppercase">Ganancia/pérdida</div>'
...
def _detail_card(
    ...
    changes = "".join(
        ...
        for label, val, fmt_val in [
            ("1 día",      data["chg_1d"],  fmt_pct(data["chg_1d"])),
            ("1 mes",      data["chg_1m"],  fmt_pct(data["chg_1m"])),
            ("3 meses",    data["chg_3m"],  fmt_pct(data["chg_3m"])),
            ("Año actual", data["chg_ytd"], fmt_pct(data["chg_ytd"])),
        ]
    )
    return (
        ...
        f'<h4 style="margin:0 0 10px;color:#1B2A4A;font-size:12px;text-transform:uppercase">Análisis técnico</h4>'
        f'<table style="width:100%;font-size:13px;border-collapse:collapse">'
        f'<tr><td style="color:#666;padding:3px 0">Máx 52 semanas</td><td style="text-align:right;font-weight:600">{data["high_52w"]:.2f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Mín 52 semanas</td><td style="text-align:right;font-weight:600">{data["low_52w"]:.2f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Media móvil 50d</td><td style="text-align:right;font-weight:600">{ma50_str}</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Media móvil 200d</td><td style="text-align:right;font-weight:600">{ma200_str}</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Volatilidad anual</td><td style="text-align:right;font-weight:600">{data["annual_vol"]:.1%}</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Caída vs máx 52s</td>'
        f'<td style="text-align:right;font-weight:600;color:{color_fn(data["drawdown"])}">{fmt_pct(data["drawdown"])}</td></tr>'
        f'</table></div>'
        f'<div style="background:#F5F8FF;border-radius:8px;padding:14px">'
        f'<h4 style="margin:0 0 10px;color:#1B2A4A;font-size:12px;text-transform:uppercase">Tu posición</h4>'
        f'<table style="width:100%;font-size:13px;border-collapse:collapse">'
        f'<tr><td style="color:#666;padding:3px 0">Participaciones</td><td style="text-align:right;font-weight:600">{cfg["participaciones"]:.4f}</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Precio medio compra</td><td style="text-align:right;font-weight:600">{avg_cost_str}</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Valor actual</td><td style="text-align:right;font-weight:600">{portfolio_val:,.2f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Ganancia/pérdida €</td>'
        ...
        f'<tr><td style="color:#666;padding:3px 0">Aportación mensual</td>'
        f'<td style="text-align:right;font-weight:600">{current_contribution(fund_id):.0f} €/mes</td></tr>'
        f'</table></div></div>'
        f'{_projection_block(proj, cfg["color"])}'
        f'{_tax_block(tax)}'
        f'<h4 style="margin:20px 0 10px;color:#1B2A4A;font-size:12px;text-transform:uppercase">Señales detectadas</h4>'
...
def build_report(results: Dict[str, Tuple[Dict[str, Any], Dict[str, Any]]]) -> str:
    ...
    total_block = (
        ...
        f'<p style="color:#8EA8C8;font-size:12px;margin:0 0 8px;text-transform:uppercase;letter-spacing:1px">💰 Dinero Total en ETFs</p>'
        f'<div style="font-size:48px;font-weight:800;color:white;margin:0">{total:,.0f} €</div>'
        f'<p style="color:#8EA8C8;font-size:12px;margin:8px 0 0">Actualizado: {_html.escape(timestamp)}</p>'
    ...
    status_block = (
        ...
        f'📊 Estado actual de la cartera</h2>'
    ...
    detail_block = (
        f'<div style="background:#1B2A4A;border-radius:12px;padding:20px 24px;margin-bottom:8px">'
        f'<h2 style="color:white;font-size:16px;margin:0;text-transform:uppercase;letter-spacing:1px">'
        f'🔍 Análisis detallado</h2></div>{detail_cards}'
    )
    ...
    if is_phase2:
        phase_block = (
            ...
            f'<strong style="color:#1A7A4A;font-size:15px">✅ FASE 2 — ETFs acelerados</strong>'
            f'<p style="margin:8px 0 0;font-size:13px;color:#333">TR lleno · Todo el ahorro mensual va a ETFs.</p></div>'
        )
    else:
        ...
        phase_block = (
            ...
            f'<strong style="color:#856404;font-size:15px">⏳ FASE 1 — Rellenando Trade Republic</strong>'
            f'<p style="margin:8px 0 0;font-size:13px;color:#333">'
            f'Faltan <strong>{months_left} meses</strong> para Fase 2 ({phase2_date}).</p>'
    ...
    plan_block = (
        f'<div style="background:#1B2A4A;border-radius:12px;padding:20px 24px;margin-bottom:8px">'
        f'<h2 style="color:white;font-size:16px;margin:0;text-transform:uppercase;letter-spacing:1px">'
        f'🗺️ Plan de inversión — Estado actual</h2></div>'
        ...
        f'<h4 style="margin:0 0 10px;font-size:12px;text-transform:uppercase;color:#1B2A4A">Hito proyectado — Año {min(year_num,10)}</h4>'
        f'<table style="width:100%;font-size:13px;border-collapse:collapse">'
        f'<tr><td style="color:#666;padding:3px 0">Valor total ETFs esperado</td><td style="text-align:right;font-weight:700">{milestone_total:,.0f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Valor total ETFs actual</td><td style="text-align:right;font-weight:700">{total_val:,.0f} €</td></tr>'
        f'<tr><td style="color:#666;padding:3px 0">Ganancia/pérdida total</td>'
        ...
        f'<h4 style="margin:0 0 10px;font-size:12px;text-transform:uppercase;color:#1B2A4A">Aportaciones actuales</h4>'
        f'<table style="width:100%;font-size:13px;border-collapse:collapse">{contributions}</table></div></div>'
    )

    disclaimer = (
        '<div style="background:#FFF3CD;border-radius:10px;padding:16px 20px;margin-top:4px;border-left:4px solid #F0A500">'
        '<strong style="color:#856404;font-size:12px">⚠️ Aviso importante</strong>'
        '<p style="margin:6px 0 0;font-size:12px;color:#444;line-height:1.6">'
        'Este monitor usa señales técnicas como indicadores de alerta, no como predicciones. '
        'Para ETF de largo plazo, la acción correcta en corrección es mantener y seguir aportando.</p></div>'
    )
...
<head>
    ...
    <title>Monitor ETF — {timestamp}</title>
    <style>
...
<div style="max-width:900px;margin:0 auto;padding:24px 16px">
    <div style="background:#1B2A4A;border-radius:12px;padding:24px 28px;margin-bottom:24px;color:white;
        ...
        <div>
            <h1 style="font-size:22px;margin-bottom:4px">📈 Monitor de Cartera ETF</h1>
